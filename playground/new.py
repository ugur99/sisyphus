...
class KubeConfigGen():
    ...
    def generate_csr_yaml(self):
        ...
        csrdata = open(inventory + ".csr","r").read()
    ...
    def generate_kubeconfig(self,cert):

        os.chdir(userKubeConfig)
        shutil.copyfile(templateDir + 'kubeconf-template.yaml', self.userName + "-" +self.clusterName + "-kubeconfig")
        subprocess.call(['chmod +x /app/playground/helper.sh'], shell=True)
        subprocess.call(['./app/playground/helper.sh ' + self.userName + " " + self.groupName + " " + self.clusterName ], shell=True)
...
#kubeConfigg = KubeConfigGen("can","minikube")

#if kubeConfigg.path_control():
#    kubeConfigg.generate_kubeconfig(kubeConfigg.k8s(kubeConfigg.generate_csr_yaml()))